...
function logClaudeCache(label, usage) {
  if (!usage) return;
  console.log(`[${label}] Claude cache — created:${usage.cache_creation_input_tokens || 0} read:${usage.cache_read_input_tokens || 0} uncachedInput:${usage.input_tokens || 0} output:${usage.output_tokens || 0}`);
}
...
function synthesizeToBufferOnce(text, timeoutMs = 8000, attempt = 1) {
  return new Promise((resolve, reject) => {
    ...
    console.log(`[TTS] attempt ${attempt} started`);
        ...
        try {
          ...
          if (googleRes.statusCode !== 200) {
            ...
            const detail = parsed && parsed.error
              ? `${parsed.error.status || 'Google TTS error'}: ${parsed.error.message || JSON.stringify(parsed.error)}`
              : `Google TTS HTTP ${googleRes.statusCode}: ${data.slice(0, 500)}`;
            console.warn(`[TTS] attempt ${attempt} failed in ${Date.now() - startedAt}ms — ${detail}`);
            return reject(new Error(detail));
          }

          if (!parsed.audioContent) {
            settled = true;
            const detail = 'Google TTS returned no audioContent';
            console.warn(`[TTS] attempt ${attempt} failed in ${Date.now() - startedAt}ms — ${detail}`);
            return reject(new Error(detail));
          }

          settled = true;
          console.log(`[TTS] attempt ${attempt} succeeded in ${Date.now() - startedAt}ms`);
          resolve(Buffer.from(parsed.audioContent, 'base64'));
        } catch (err) {
          settled = true;
          console.warn(`[TTS] attempt ${attempt} parse failure in ${Date.now() - startedAt}ms — ${err.message}`);
          reject(err);
        }
      });
    });
    ...
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      console.warn(`[TTS] attempt ${attempt} timed out after ${timeoutMs}ms`);
      googleReq.destroy(new Error(`TTS timeout after ${timeoutMs}ms`));
    ...
    googleReq.on('error', err => {
      ...
      if (settled) return;
      settled = true;
      console.warn(`[TTS] attempt ${attempt} request error in ${Date.now() - startedAt}ms — ${err.message}`);
      reject(err);
    });

    googleReq.on('close', clearTimer);
...
async function synthesizeToBuffer(text) {
  ...
  try {
    return await synthesizeToBufferOnce(text, 8000, 1);
  } catch (firstErr) {
    console.warn(`[TTS] retrying once after attempt 1 — ${firstErr.message}`);
    return await synthesizeToBufferOnce(text, 8000, 2);
  }
}
...
app.post('/claude-tts', (req, res) => {
  ...
  const claudeReq = https.request(claudeOptions, (claudeRes) => {
    ...
    claudeRes.on('end', async () => {
      try {
        const parsed = JSON.parse(data);
        logClaudeCache('claude-tts', parsed.usage);
    ...
    fullText = await new Promise((resolve, reject) => {
      ...
      const claudeReq = https.request(options, (claudeRes) => {
        ...
        claudeRes.on('end', () => {
          try {
            const parsed = JSON.parse(data);
            logClaudeCache('stt-claude-tts', parsed.usage);
            if (claudeRes.statusCode !== 200) {
              conversationHistory.pop();
...
app.post('/claude', (req, res) => {
  ...
  const claudeReq = https.request(options, (claudeRes) => {
    ...
    claudeRes.on('end', () => {
      try {
        const parsed = JSON.parse(data);
        logClaudeCache('claude', parsed.usage);
        if (claudeRes.statusCode !== 200) {
          conversationHistory.pop();
          return res.status(claudeRes.statusCode).json({ error: 'Claude API error', detail: parsed });
        }
...
async function handleSongUpload(req, res) {
  ...
  const file       = req.file;
  // The upload page sends the selected file name (without its extension) as the
  // authoritative song title. This prevents AI analysis from renaming a song.
  const submittedSongTitle = ((req.body && req.body.songTitle) || '').trim().slice(0, 180);
  ...
  if (submittedSongTitle) session.songTitle = submittedSongTitle;
  ...
  try {
    ...
    // If the upload supplied a title, it always wins. Claude may infer a title
    // only when the client did not provide one (for example pasted text).
    session.songTitle = submittedSongTitle || session.songTitle || parsed.songTitle || '';
...
${songTitle ? `AUTHORITATIVE SONG TITLE FROM THE UPLOAD: "${songTitle}". Copy this EXACTLY into songTitle. Do NOT rename, correct, reinterpret, or replace it based on the page contents.` : 'No authoritative title was supplied; infer songTitle only if it is clearly visible.'}
...
app.listen(PORT, () => {
  ...
  console.log('TTS retry: enabled — 8s timeout, 1 retry');
});
